...
class Plugin:
    ...
    async def _eval_js(self, expression: str):
        ...
        try:
            ...
            await decky.emit("timer_event", f"Connecting to {ws_url}")
            ...
            try:
                ...
                res = ws.recv()
                await decky.emit("timer_event", "injection result")
                return res
            finally:
                ws.close()
        except Exception as e:
    ...
    async def start_timer(self, css: str):
        ...
        try:
            ...
            await decky.emit("timer_event", f"Translated CSS len {len(translated_css)}")
            ...
            (function(){{
                try {{
                    if (!document.getElementById("decky_inject_style")) {{
            ...
            await decky.emit("timer_event", "injecting translated css once..")
            await self._eval_js(js)
            await decky.emit("timer_event", "injected (once)")
        except Exception as e:
